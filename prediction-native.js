/* 맞혀도랑 v6.2.0 통합 모듈 · iframe 없이 Shadow DOM에서 실행 */
(()=>{
...
function blank(){return {tournament:'새 대회',teamA:'',teamB:'',participants:[],matches:[],current:-1,playerRaces:{},playerTiers:{},version:31}}
let data=load();
...
function restoreData(e){let f=e.target.files[0];if(!f)return;let r=new FileReader();r.onload=()=>{try{let x=JSON.parse(r.result);if(!Array.isArray(x.participants)||!Array.isArray(x.matches))throw 0;data=x;ensurePlayerMetaMemory();seedPlayerMetaMemory();save()}catch(_){alert('올바른 백업 파일이 아니야.')}};r.readAsText(f)}
...
window.PredictionNative={addMatch,addScheduleRow,autoPlayerMeta,clearWinner,closeModal,deleteCurrentMatch,deleteScheduleRow,duplicateMatch,exportData,moveScheduleRow,nextMatch,openEventSettings,openNewMatch,openParticipants,openSchedule,pick,prevMatch,quickAddParticipant,resetTournament,restoreData,saveEventSettings,saveParticipants,saveQuickParticipant,saveSchedule,schedulePlayerInput,selectHistory,setScheduleDraft,setWinner,toggleLock,toggleMissingOnly};
render();
  return host;
};
